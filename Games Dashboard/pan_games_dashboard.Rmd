...
overall_attempted <- participant_dates %>%
  select(hml_id, participant_id_parent, area)
...
overall_attempted_download <- overall_attempted %>%
  select(hml_id, participant_id_parent, area, ends_with("game_status")) %>%
    ...
    if(game == "All"){ # Look at all games for all areas
     overall_attempted_table %>%
        ...
        mutate(counts = paste0(number_missing, " (", round(percent_missing, 1), "%)"),
               area = paste0(str_to_title(area), " (n = ", total_games, ")")) %>%
    ...
    if(game == "All"){
      if(picked_id == "All"){ # Look at list of completed games for specific area
        ...
        overall_attempted %>%
          ...
          select(-c(hml_id, participant_id_parent, area)) %>%
      ...
      files_list[[game]] %>%
        filter(hml_id == picked_id) %>%
        select(-c(hml_id, participant_id, study_start_date_before, study_start_date_after, area, game_result, game_name, browser_useragent,
