...
# 02-May-24: Google drive changed link to data from 1qa2MeTjZMNQQep4EIPOh0MMKCY2-M8Jc to 1n4LtM6Z3YzmP40K5Vpm4asdCaRcbjcdY
...
overall_attempted <- files_list[[1]] %>%
  select(study_id, area, contains("game_status"), contains("timestamp")) %>%
  group_by(study_id) %>% arrange(.[3]) %>% slice(1) %>% ungroup()

for(i in 2:length(files_list)){
  
  game_data <- files_list[[i]] %>% 
    select(study_id, contains("game_status"), contains("timestamp")) %>%
    group_by(study_id) %>% arrange(.[3]) %>% slice(1) %>% ungroup()
...
table_function <- function(picked_area = "All", game = "All", picked_id = "All"){
  if(picked_area == "All"){
    ...
    if(game == "All"){
      ...
      files_list[[game]] %>%
        filter(study_id == picked_id) %>%
        select(-c(study_id, area, game_result, game_name, browser_useragent,
...
download_data_function <- function(picked_id){
  ...
  redcap_data_participant <- lapply(redcap_data, function(x){
    new_x <- filter(x, study_id == picked_id)
    new_x
  })
...
redcap_data_participant <- lapply(redcap_data, function(x){
    new_x <- filter(x, study_id == "HML0104")})
...
#### Data updated: `r games_update_date`.
