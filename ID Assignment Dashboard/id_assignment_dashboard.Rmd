...
if(production){
  ...
  # id_dat <- dbReadTable(con, "dm_hml_id_app_data"); write.csv(id_dat, file = "test_hml_id_app_data.csv",row.names = FALSE)
  ...
  withProgress(message = "Saving... Please Wait", {
    ...
    if(production){
      ...
      if(nrow(updated_ids) > 0){
        # Remove any whitespaces from ID
        updated_ids$participant_id <- stringr::str_trim(updated_ids$participant_id, side = "both")
