...
# Mindcrowd Prescreen population
prescreen_data <- data %>%
...
prescreen_data <- mutate(prescreen_data,
                             race_group = ifelse(race_group == "Hispanic", "Hispanic/Latino", race_group))
...
#### Last MindCrowd test taken: `r format(max(all_mindcrowd_data$day_time), "%b-%d-%Y")`
...
#### Last MindCrowd test taken within MindCrowd prescreen eligible: `r format(max(prescreen_data$day_time), "%b-%d-%Y")`
...
#### Data were last pulled from REDCap on: `r format(max(hml_data$update_date), "%b-%d-%Y")`
