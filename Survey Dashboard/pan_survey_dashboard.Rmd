...
# Add new QPAR questions to data dictionary
survey_data_dictionary <- survey_data_dictionary %>%
  rbind(
    c("QPAR_v1.0.17", "Engage_in_Moderate_Housework_or_Labor", "", "QPAR Survey")
    ,c("QPAR_v1.0.18", "Hours_Engage_in_Moderate_Housework_or_Labor", "", "QPAR Survey")
    ,c("QPAR_v1.0.19", "Engage_in_Heavy_Housework_or_Labor", "", "QPAR Survey")
    ,c("QPAR_v1.0.20", "Hours_Engage_in_Heavy_Housework_or_Labor", "", "QPAR Survey")
  )
...
consent_dat <- dbGetQuery(con, "SELECT hml_id, main_consent_date FROM p2_redcap_consent_form WHERE visit = 1") %>%
  ...
  mutate(first_visit_date = ifelse(is.na(main_consent_date), hml_id_created_date, main_consent_date),
         ...
         first_visit = 1,
         second_visit = ifelse(as.numeric(Sys.Date() - as.Date(first_visit_date)) >= 365*2-60, 2, NA)) %>%
...
hml_ids <- hml_ids %>%
  ...
  mutate(survey_visit_number = ifelse(is.na(first_visit_date), 1, survey_visit_number),
         first_visit_date = ifelse(is.na(first_visit_date), as.character(Sys.Date()), first_visit_date))
...
raw_files_list <- lapply(survey_names, 
                         function(x){
                           ...
                           # 03-Nov-2025 temp fix: HML ID originally submitted with created date instead of MC ID
                           new_x$hml_id[which(new_x$participant_id == "003Vu00000yhm8tIAA")] <- "HML1191"
                           new_x$record_id[which(new_x$participant_id == "003Vu00000yhm8tIAA")] <- "1178"
                           new_x$site[which(new_x$participant_id == "003Vu00000yhm8tIAA")] <- "Miami"
                           new_x$p2_participant[which(new_x$participant_id == "003Vu00000yhm8tIAA")] <- "True"
                           
                           # 11-May-2026: Participants logged into MC under different emails than what
                           # was used to assign HML IDs.
                           new_x$hml_id[which(new_x$participant_id == "003Vu00001tEcN1IAK")] <- "HML1318"
                           new_x$record_id[which(new_x$participant_id == "003Vu00001tEcN1IAK")] <- "1303"
                           new_x$site[which(new_x$participant_id == "003Vu00001tEcN1IAK")] <- "Tucson"
                           new_x$p2_participant[which(new_x$participant_id == "003Vu00001tEcN1IAK")] <- "True"
                           
                           # 16-Jul-2026 Incorrect MCIDs were registered to these HML IDs
                           new_x$hml_id[which(new_x$participant_id == "003Vu000016eCZeIAM")] <- "HML1147"
                           new_x$record_id[which(new_x$participant_id == "003Vu000016eCZeIAM")] <- "1132"
                           new_x$site[which(new_x$participant_id == "003Vu000016eCZeIAM")] <- "Atlanta"
                           new_x$p2_participant[which(new_x$participant_id == "003Vu000016eCZeIAM")] <- "True"
                           
                           new_x$hml_id[which(new_x$participant_id == "003Vu00000P6tprIAB")] <- "HML1211"
                           new_x$record_id[which(new_x$participant_id == "003Vu00000P6tprIAB")] <- "1196"
                           new_x$site[which(new_x$participant_id == "003Vu00000P6tprIAB")] <- "Atlanta"
                           new_x$p2_participant[which(new_x$participant_id == "003Vu00000P6tprIAB")] <- "True"
...
files_list <- lapply(raw_files_list_edit, function(x){
  ...
  recent_x <- complete_x %>%
    ...
    # 14-Oct-25: HML1152 is missing first_visit_date??
    mutate(first_visit_date = ifelse(is.na(first_visit_date), as.character(Sys.Date()), first_visit_date)) %>%
    mutate(
      ...
      survey_visit_number = ifelse(as.numeric(time_since_first_visit) >= 365*2-90, 2, 1)
    ) %>% 
    ...
    if (survey == "All") { # Look at all surveys for specific participant
      ...
      if (visit == 2 & survey %in% remove_from_second_visit) {
        ...
        redcap_table_dat <- formatted_redcap_import_files_list[[survey]] %>%
          rename(survey_visit_number = visit) %>%
          filter(hml_id == picked_id, survey_visit_number == as.double(visit)) %>%
