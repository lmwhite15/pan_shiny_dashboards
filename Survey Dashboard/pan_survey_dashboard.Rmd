  ...
  if (picked_site == "All") {
    ...
    if (survey == "All") {
      overall_attempted_time %>%
        ...
        kable(col.names = c("Survey", "Date Attempted", "Completion Status")) %>%
        kable_styling(full_width = F)
...
download_data_function <- function(picked_id, visit){
  ...
  withProgress(message = "Creating REDCap Data... Please Wait", {
    ...
    if(visit != 1) redcap_files_list <- redcap_files_list[!(names(redcap_files_list) %in% c("brain_disease", "covid", "fhad", "health_medical", "ses", "subjective_english"))]
